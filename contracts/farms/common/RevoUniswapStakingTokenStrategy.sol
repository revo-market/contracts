...
abstract contract RevoUniswapStakingTokenStrategy is StakingTokenHolder {
    ...
    function compound(
        ...
        for (uint256 i = 0; i < rewardsTokens.length; i++) {
            ...
            require(address(rewardsTokens[i]) == _paths[i][0][0], "invalid path start");
            require(address(stakingToken0) == _paths[i][0][_paths[i].length - 1], "invalid path end");
            require(address(rewardsTokens[i]) == _paths[i][1][0], "invalid path start");
            require(address(stakingToken1) == _paths[i][1][_paths[i].length - 1], "invalid path end");
        }
